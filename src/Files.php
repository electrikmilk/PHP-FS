<?php
/*
 * Created by @electrikmilk, 01-21-2021
...
class Files
{
    ...
    private function __construct(string $path = null, bool $absolute = true)
    {
        ...
        try {
            ...
            if (!file_exists($path)) {
                throw new Exception("Path '$path' does not appear to exist.");
            }
            if (!is_readable($path)) {
                throw new Exception("Path '$path' does not appear to be readable.");
            }
            if (!is_writeable($path)) {
                trigger_error("Path '$path' appears to be read-only.", E_USER_NOTICE);
            }
            if ($absolute === true) {
    ...
    public function copy($item, $newpath = null, $force = false)
    {
        ...
        if (file_exists($path)) {
            return $this->transfer("copy", $path, $newpath, $force);
        } else {
            return null;
        }
    }
    public function move($item, $newpath, $force = false)
    {
        ...
        if (file_exists($path)) {
            return $this->transfer("move", $path, $newpath, $force);
        } else {
            return null;
        }
    }
    ...
    { // TODO: make it so you could write a funciton like this copy("file","directory"), not copy("file","directory/file");
        if (is_file($path)) {
            ...
            return $this->copy_dir($path, $newpath);
        }
    }
